            ...
            <ul className="space-y-3">
              {footerLinks.product.map((link) => (
                <li key={link.name}>
                  ...
                  >
                    <span className="bg-gradient-to-r from-transparent to-transparent hover:from-primary/50 hover:to-transparent bg-[length:0%_1px] hover:bg-[length:100%_1px] bg-no-repeat bg-left-bottom transition-all duration-500">
                      {link.name}
                    </span>
                  </Link>
                </li>
            ...
            <ul className="space-y-3">
              {footerLinks.social.map((link) => (
                <li key={link.name}>
                  <Link 
                    href={link.href}
                    className="text-sm text-white/60 hover:text-white/90 transition-colors inline-flex items-center gap-2 group"
                  >
                    <span className="bg-white/5 p-1.5 rounded-md group-hover:bg-white/10 transition-colors">
                      {link.icon}
                    </span>
                    <span className="bg-gradient-to-r from-transparent to-transparent hover:from-primary/50 hover:to-transparent bg-[length:0%_1px] hover:bg-[length:100%_1px] bg-no-repeat bg-left-bottom transition-all duration-500">
                      {link.name}
        ...
        <div className="mt-12 pt-8 border-t border-white/10">
          <div className="flex flex-col items-center space-y-4 sm:flex-row sm:justify-between sm:space-y-0">
            <p className="text-xs text-white/60 text-center sm:text-left">
              © {new Date().getFullYear()} Audiq. All rights reserved.
            </p>
            <div className="flex flex-wrap justify-center gap-x-6 gap-y-2">
              {["Privacy Policy", "Terms of Service", "Cookie Policy"].map((text) => (
                <Link
                  key={text}
                  href="#"
                  className="text-xs text-white/60 hover:text-white/90 transition-colors inline-flex items-center gap-2"
                >
                  <span className="bg-gradient-to-r from-transparent to-transparent hover:from-primary/50 hover:to-transparent bg-[length:0%_1px] hover:bg-[length:100%_1px] bg-no-repeat bg-left-bottom transition-all duration-500">
                    {text}
                  </span>
